...
function renderTable( ?string $table, array $columns, ?string $primary ): void {
	if ( ! $table ) return; // skip rendering if no table selected
	?>
	...
	<table border="1" cellpadding="5">
		...
		<?php
			renderInputRow( $columns );
			renderTableRows( $table, $columns, $primary,  );
		?>
...
function renderInputRow( array $columns ): void {
	?>
	<tr>
		<form method="post">

			<?php foreach ( $columns as $col ):
				$flags = getColumnFlags( $col ); // get flags for rendering decisions
			?>
				<td>
					...
					<?php else: ?>
						<input name="<?= htmlspecialchars( $flags['name'] ) ?>" />
					<?php endif; ?>
				</td>
			<?php endforeach; ?>

			<!-- Submit button for adding new row -->
